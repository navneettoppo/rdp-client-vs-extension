    ...
    let disposable = vscode.commands.registerCommand('extension.connectRDP', async () => {
        ...
        if (address && username && password) {
            const panel = vscode.window.createWebviewPanel(
                'rdpClient', // Identifies the type of the webview. Used internally
            ...
            panel.webview.onDidReceiveMessage(async message => {
				if (message.command === 'connect') {
					try {
						...
						connection.then(function(deferred: any) {
...
function getWebviewContent(address: string, username: string) {
    return `
        <!DOCTYPE html>
        <html lang="en">
        <head>
